...
function SlantedPanel({
  ...
  titleSize = 24, // px default
  subtitleSize = 16, // px default
  descriptionSize = 20, // px default
  ...
  // Apply title/subtitle font-size overrides if provided (coerce to safe numbers)
  try {
    const ts = Number(titleSize) || 0;
    if (ts > 0) titleStyle.fontSize = `${Math.max(8, Math.min(120, Math.round(ts)))}px`;
  } catch (e) {
    // ignore invalid values
  }

  try {
    const ss = Number(subtitleSize) || 0;
    if (ss > 0) subtitleStyle.fontSize = `${Math.max(8, Math.min(120, Math.round(ss)))}px`;
  } catch (e) {
    // ignore invalid values
  }
  ...
  return (
    <div className="slanted-panel-container" style={customStyles.container}>
      <div className="slanted-panel" style={panelStyle}>
        {/* Left emote area */}
        <div
          className={`panel-left-well ${isTransitioning ? "transitioning" : ""} animation-${animation}`}
          style={customStyles.leftWell}
        >
          {emote &&
            ...
            ((emote.includes(".") || emote.startsWith("http")) && !imageError ? (
              ...
              <div className="panel-emote" style={emoteTextStyle}>
                {emote}
              </div>
            ))}
        </div>
        ...
        {/* Right content area */}
        <div
          className={`panel-right-content ${isTransitioning ? "transitioning" : ""} animation-${animation}`}
          style={customStyles.rightContent}
        >
          {title && (
            <h2 className="panel-title" style={titleStyle}>
              <span
                ...
                style={
                  titleMarquee
                    ? { "--marquee-to": `-${titleMarquee.distance}px`, animationDuration: `${titleMarquee.duration}s` }
                    : {}
                }
              >
                {title}
            ...
            <p className="panel-subtitle" style={subtitleStyle}>
              <span
                ...
                style={
                  subtitleMarquee
                    ? {
                        "--marquee-to": `-${subtitleMarquee.distance}px`,
                        animationDuration: `${subtitleMarquee.duration}s`,
                      }
                    : {}
                }
              >
